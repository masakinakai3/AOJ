...
int main() {
    ...
    vector<Dice> input_data(n);
    for(int i=0;i<n;i++){
      int s1,s2,s3,s4,s5,s6;
      cin>>s1>>s2>>s3>>s4>>s5>>s6;
      input_data.at(i).SetDice(s1,s2,s3,s4,s5,s6);
    }

    Dice wk;
    bool comp_all_different=false;
    bool comp_same=false;
    for(int i=0;i<n-1;i++){
      for(int j=i+1;j<n;j++){
        bool flg_wk=wk.Comparison_dice(input_data.at(i),input_data.at(j));
        if(flg_wk)
          comp_same=true;
        else{
          if(i==n-2&&j==n-1&&comp_same==false){
            comp_all_different = true;
          }
        }
      }
    }
